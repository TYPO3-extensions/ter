...
function writeExtensionAndIconFile(&$extensionData, $filesData)
{
    ...
    if (@!is_dir($GLOBALS['repositoryDir'] . $firstLetter)) {
        mkdir($GLOBALS['repositoryDir'] . $firstLetter);
    }
    if (@!is_dir($GLOBALS['repositoryDir'] . $firstLetter . '/' . $secondLetter)) {
        mkdir($GLOBALS['repositoryDir'] . $firstLetter . '/' . $secondLetter);
    }
...
function writeExtensionInfoToDB($accountData, $extensionData, $filesData, $extensionVersionRow, $versionOfExtension)
{
    ...
    foreach ($filesData as $fileData) {
        $extensionData['infoData']['files'][$fileData['name']] = [
            'name' => $fileData['name'],
            'size' => $fileData['size'],
            'mtime' => $fileData['mtime'],
            'is_executable' => $fileData['is_executable'],
 ...
 *
 * @param    mixed        Input string, then a version number like "3.6.0" or "3.4.5rc2". If double it will be converted back to
 *     string. Version numbers after suffix is not supported higher than "9".
